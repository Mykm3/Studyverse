...
export function PlanModal({ open, onClose, onSubmit, isLoading = false }) {
  ...
  const calculateSessionLimits = () => {
    ...
    const basePerSubject = Math.floor(totalSlots / selectedSubjects.length);
    
    return {
      totalSlots,
      basePerSubject,
      maxSessionsPerDay,
      recommendedMax: Math.min(basePerSubject, 4) // Cap at 4 sessions per subject per week
    };
  };
  ...
  const validateSessionCapacity = () => {
    ...
    return {
      ...
      recommendedMaxPerSubject: sessionLimits.recommendedMax,
  ...
  const updateSubjectSessions = (subjectName, sessionsPerWeek) => {
    setSubjectSessions(prev => ({ ...prev, [subjectName]: sessionsPerWeek }));
  };
                ...
                <div className="grid grid-cols-1 gap-2">
                  {availableSubjects.map((subject) => (
                   ...
                   >
                     ...
                     {capacityValidation.recommendedMaxPerSubject > 4 && (
